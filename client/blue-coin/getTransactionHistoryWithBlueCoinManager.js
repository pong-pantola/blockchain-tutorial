...
async function main(orgIndex, userIndex, mspId){
  ...
  const response = await bcMgr.getTransactionHistory([mspId], `user${userIndex}`, 
    function(err, txId, status, blockNo){
      if (err)
        console.err(err);
      else{
        console.log("getTransactionHistory invocation successful.")
...
if (process.argv.length != 5){
  console.log("Syntax : node getTransactionHistoryWithBlueCoinManager.js <org index> <user index> <mspId>")
  console.log("Example: node getTransactionHistoryWithBlueCoinManager.js 1 2 Org1MSP")
}else{
