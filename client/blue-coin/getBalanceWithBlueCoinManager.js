...
async function main(orgIndex, userIndex, mspId){
  ...
  const response = await bcMgr.getBalance([mspId], `user${userIndex}`, 
    function(err, txId, status, blockNo){
      if (err)
        console.err(err);
      else{
        console.log("getBalance invocation successful.")
...
if (process.argv.length != 5){
  console.log("Syntax : node getBalanceWithBlueCoinManager.js <org index> <user index> <mspId>")
  console.log("Example: node getBalanceWithBlueCoinManager.js 1 2 Org1MSP")
}else{
